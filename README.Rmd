...
### KPthorSQL
The DSN name used for production is `KPthorSQL`, but supports a `development` mode which uses the `KPthorSQL-dev` DSN name. Add the following entry to `/etc/odbc.ini` (`~/.odbc.ini` on macOS), and fill in your specific connection details:
...
### ODBC
The ODBC driver needs to downloaded from the internet. Add the following entry to `/etc/odbcinst.ini`:
